...
namespace DimBoxes
{
    ...
    /*public enum w_placement: int { top_front = 0, bottom_front = 5, bottom_back = 6, top_back = 7 };
    public enum d_placement: int { top_right = 0, bottom_right = 5, bottom_left = 6, top_left = 7 };
    public enum h_placement: int { front_right = 0, front_left = 5, back_left = 6, back_right = 7 };*/
    ...
    public class DimBox : MonoBehaviour
    {
        ...
        public class Formatter
        {
            ...
            public bool captionMode = false;
            public CaptionSettings captionSettings;
            public string Format(float f)    
            {
                return (factor*f).ToString(format) + " " + unit;
            }
        ...
        {
            public string heightCaption = "height caption";
            public string widthCaption = "width caption";
            public string depthCaption = "depth caption";
        }
        ...
        [HideInInspector]
        [SerializeField]
        private TextMesh htm;
        [HideInInspector]
        [SerializeField]
        private TextMesh dtm;
        [HideInInspector]
        [SerializeField]
        private TextMesh wtm;
...
//Can not use dll's attached to MathGeoLib in WebGL
        ...
        void AddText()
        {
...
#endif
            hDimensionMesh.transform.parent = PossiblePlacementsHeight[i];// hDimension.transform;
            ...
            htm.text = formatter.captionMode? formatter.captionSettings.heightCaption : formatter.Format(bound.size.y * scale.y);
...
#endif
                wDimensionMesh.transform.parent = PossiblePlacementsWidth[i]; //wDimension.transform;
            ...
            wtm.text = formatter.captionMode ? formatter.captionSettings.widthCaption : formatter.Format(bound.size.x * scale.x);
...
#endif
                dDimensionMesh.transform.parent = PossiblePlacementsDepth[i]; //dDimension.transform;
            ...
            dtm.text = formatter.captionMode ? formatter.captionSettings.depthCaption : formatter.Format(bound.size.z * scale.z);
        ...
        void SortDimPositions()
        {
            ...
            Vector3 CPoint = Camera.main.WorldToScreenPoint(transform.TransformPoint(boundOffset));
            float hDist = 0, dDist = 0, wDist = 0;
            int newHpos = 0, newDpos = 0, newWpos = 0;
            for (int i = 0; i < 4; i++)
            {
                Vector3 hPoint = Camera.main.WorldToScreenPoint(transform.TransformPoint(EdgesHeight[i]));
                Vector3 wPoint = Camera.main.WorldToScreenPoint(transform.TransformPoint(EdgesWidth[i]));
                Vector3 dPoint = Camera.main.WorldToScreenPoint(transform.TransformPoint(EdgesDepth[i]));
                if (i == 0)
                {
                    hDist = hPoint.x;
                    wDist = wPoint.y;
            ...
            if (faceCamera.height)
            {
                ...
                if (extensions.height)
                {
                    Vector3 h0Point = Camera.main.WorldToScreenPoint(PossiblePlacementsHeight[newHpos].transform.position);
                    //Debug.DrawRay(campos, PossiblePlacementsHeight[newHpos].transform.position - campos, Color.red);
                    Vector3 h1Point = Camera.main.WorldToScreenPoint(PossiblePlacementsHeight[(newHpos + 1) % 8].transform.position);
            ...
            hDimensionMesh.transform.parent = PossiblePlacementsHeight[newHpos];
            hDimensionMesh.transform.localPosition = Vector3.zero;
            ...
            if (faceCamera.width)
            {
                ...
                if (extensions.width)
                {
                    Vector3 w0Point = Camera.main.WorldToScreenPoint(PossiblePlacementsWidth[newWpos].transform.position);
                    Vector3 w1Point = Camera.main.WorldToScreenPoint(PossiblePlacementsWidth[(newWpos + 1) % 8].transform.position);
            ...
            wDimensionMesh.transform.parent = PossiblePlacementsWidth[newWpos];
            wDimensionMesh.transform.localPosition = Vector3.zero;        
            ...
            if (faceCamera.depth)
            {
                ...
                if (extensions.depth)
                {
                    Vector3 d0Point = Camera.main.WorldToScreenPoint(PossiblePlacementsDepth[newDpos].transform.position);
                    Vector3 d1Point = Camera.main.WorldToScreenPoint(PossiblePlacementsDepth[(newDpos + 1) % 8].transform.position);
            ...
            dDimensionMesh.transform.parent = PossiblePlacementsDepth[newDpos];
            dDimensionMesh.transform.localPosition = Vector3.zero;
        ...
        private void LateUpdate()
        {
            ...
            if (faceCamera.height)
            {
                ...
                if(Camera.main.transform.InverseTransformDirection(hDim.up).x<0) hDim.Rotate(0,0,180f);
            }
            ...
            if (faceCamera.width)
            {
                ...
                if (Camera.main.transform.InverseTransformDirection(wDim.up).x < 0) wDim.transform.Rotate(0, 0, 180f);
            }
            ...
            if (faceCamera.depth)
            {
                ...
                if (Camera.main.transform.InverseTransformDirection(dDim.up).x < 0) dDim.transform.Rotate(0, 0, 180f);
            }
        }
